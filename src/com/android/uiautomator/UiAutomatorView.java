...
public class UiAutomatorView extends Composite {
	...
	public UiAutomatorView(Composite parent, int style) {
		...
		this.mScreenshotCanvas.addListener(SWT.MouseEnter, new Listener() {
			public void handleEvent(Event arg0) {
				UiAutomatorView.this.getShell().setCursor(UiAutomatorView.this.mCrossCursor);
			}
		});
		this.mScreenshotCanvas.addListener(SWT.MouseExit, new Listener() {
			public void handleEvent(Event arg0) {
		...
		this.mScreenshotCanvas.setBackground(getShell().getDisplay().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
		...
		TableViewerColumn tableViewerColumnValue = new TableViewerColumn(this.mTableViewer, SWT.NONE);
	...
	private void updateSearchResultSelection() {
		...
		this.itemDeleteAndInfo.setText("" + (this.mSearchResultIndex + 1) + "/" + this.mSearchResult.size());
	}
